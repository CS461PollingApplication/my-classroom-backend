...
const setUserAuthCookie = async (res, user) => {
    ...
    res.setHeader("Set-Cookie", [
        serialize("auth", generateUserAuthToken(user), {
            path: "/",
        ...
        serialize("csrf", userSession.csrfToken, { //protect from CSRF by giving the client a token that can be accessed and attached in request headers for checking
            path: "/",
            expires: new Date(Date.now() + 1000 * 60 * 60 * 12) //expires in 12 hours
        })
    ])
...
const validateCsrfToken = async (token, userId) => {
    ...
    if (session == null) {
        logger.warn(`CSRF token unauthenticated, token: ${token}`)
        throw new Error("csrf token unauthenticated")
    }
    else {
        logger.debug(`valid session found: ${session.csrfToken}, expires: ${session.expires}, id: ${session.id}`)
        const expired = session.checkIfExpired()
        if (expired) {
            logger.debug("User tried to use an expired session")
            throw new Error("user session expired")
        }
        else {
            if (session.userId != userId) {
                logger.warn(`Authenticated user and user session somehow didn't match, session.userId: ${session.userId}, userId: ${userId}`)
