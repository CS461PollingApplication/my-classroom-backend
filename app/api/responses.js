...
router.post('/', requireAuthentication, async function (req, res, next) {
    ...
    if (enrollmentStudent) {
        ...
        if (req.body.answers && Object.keys(req.body.answers).length > 1) { // request must have an answer and it must have at least two options to pick from 
            try {
                const questionInLecture = await questionService.getQuestionInLecture(questionId, lectureId)
                if (questionInLecture) { // if we found a valid questionInLecture
                    if (questionInLecture.published === true) {
                        ...
                        const response = await db.Response.create(responseService.extractResponseInsertFields(responseToInsert))
                        res.status(201).send({
                            response: responseService.extractResponseFields(response)
                        })
                    } else { // question is no longer available
                        res.status(400).send({ error: `The question is not published` })
                    }
                } else {
                    res.status(404).send({error: `No question in lecture found for the given lecture and question`})
                }
            } catch (e) {
                next(e)
            }
        ...
        res.status(403).send({error: `Only a student in the course can submit a response to the question`})
    }
})

// student is resubmitting their answer to a question
router.put('/:response_id', requireAuthentication, async function (req, res, next) {
    const user = await db.User.findByPk(req.payload.sub) // find user by ID, which is stored in sub
